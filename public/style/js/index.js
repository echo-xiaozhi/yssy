...
//	})
	...
	$(document).mousemove(function(e){   
        ...
        if( x < x1 || x > x2 || y < y1 || y > y2){
            $('.YLimgconImageLeft').css({cursor :'url("/style/img/left-b.ico"),auto'})
            $('.YLimgconImageRight').css({cursor :'url("/style/img/right-b.ico"),auto'})
            $('.YLimgconImageCenter').css({cursor :'url("/style/img/cha-w.ico"),auto'})
        }else{
            $('.YLimgconImageLeft').css({cursor :'url("/style/img/left-w.ico"),auto'})
            $('.YLimgconImageRight').css({cursor :'url("/style/img/right-w.ico"),auto'})
            $('.YLimgconImageCenter').css({cursor :'url("/style/img/cha-w.ico"),auto'})
        };
        // 小智 end
    });
